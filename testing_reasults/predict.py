...
if __name__ == '__main__': 
    ...
    candidate_weights = [
        this_dir / "best.pt",  # exported best weights (preferred)
        this_dir / "runs" / "detect" / "train" / "weights" / "best.pt",
    ]

    model_path = next((path for path in candidate_weights if path.exists()), None)
    if model_path is None:
        print("No model weights found. Place best.pt next to predict.py or under runs/detect/train/weights.")
        exit()
